...
RLAPI void
CameraPitch(Camera* camera, float angle, bool lockView, bool rotateAroundTarget, bool rotateUp);
...
// NOTE: angle must be provided in radians
void CameraPitch(
    Camera* camera, float angle, bool lockView, bool rotateAroundTarget, bool rotateUp) {
    // Up direction
    Vector3 up = GetCameraUp(camera);
...
Matrix GetCameraProjectionMatrix(Camera* camera, float aspect) {
    if (camera->projection == CAMERA_PERSPECTIVE) {
        ...
        return MatrixOrtho(
            -right, right, -top, top, CAMERA_CULL_DISTANCE_NEAR, CAMERA_CULL_DISTANCE_FAR);
    }
...
void UpdateCamera(Camera* camera, int mode) {
    ...
    if (mode == CAMERA_CUSTOM) {
        ...
        if ((mode == CAMERA_FREE) && (IsMouseButtonDown(MOUSE_BUTTON_MIDDLE))) {
            ...
            // Mouse support
            CameraYaw(
                camera, -mousePositionDelta.x * CAMERA_MOUSE_MOVE_SENSITIVITY, rotateAroundTarget);
            CameraPitch(camera,
