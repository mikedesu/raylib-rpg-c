    ...
    vec3 get_random_loc() {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                if (tile.get_cached_player_present() || tile.get_cached_live_npc() != ENTITYID_INVALID || tile.get_dead_npc_count() > 0 ||
                    tile.get_cached_item() != ENTITYID_INVALID || tile.get_cached_prop() != ENTITYID_INVALID || tile.get_cached_box() != ENTITYID_INVALID ||
                    tile.get_cached_chest() != ENTITYID_INVALID || tile.get_cached_door() != ENTITYID_INVALID) {
                    continue;
                }
                tmp.push_back(loc);
            }
