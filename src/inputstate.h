...
static inline bool inputstate_any_pressed(const std::shared_ptr<inputstate> is) {
    if (!is)
        return false; // Invalid inputstate
...
static inline bool inputstate_is_left_shift_held(std::shared_ptr<inputstate> is) {
    return inputstate_is_held(is, KEY_LEFT_SHIFT);
}

static inline bool inputstate_is_right_shift_held(std::shared_ptr<inputstate> is) {
    return inputstate_is_held(is, KEY_RIGHT_SHIFT);
}

static inline bool inputstate_is_shift_held(std::shared_ptr<inputstate> is) {
