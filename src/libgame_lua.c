...
const char* libgame_lua_get_str(lua_State* L, const char* key) {
    ...
    lua_getglobal(L, key);
    if (lua_isstring(L, -1)) retval = lua_tostring(L, -1);
    lua_pop(L, 1);
    return retval;
}
...
const int libgame_lua_get_int(lua_State* L, const char* key) {
    ...
    //if (L) {
    lua_getglobal(L, key);
    if (lua_isnumber(L, -1)) retval = lua_tonumber(L, -1);
    lua_pop(L, 1);
    //}
    return retval;
}
...
const int libgame_lua_create_entity(lua_State* L, const char* name, const int type, const int x, const int y) {
    ...
    lua_pushnumber(L, x);
    lua_pushnumber(L, y);
    if (lua_pcall(L, 4, 1, 0) == 0) retval = lua_tonumber(L, -1);
    lua_pop(L, 1);
    //}
    return retval;
}
...
const bool libgame_lua_set_entity_int(lua_State* L, const int id, const char* key, const int value) {
    ...
    if (lua_pcall(L, 3, 1, 0) == 0) retval = lua_toboolean(L, -1);
    lua_pop(L, 1);
    //}
    return retval;
}
...
const char* libgame_lua_get_entity_str(lua_State* L, const int id, const char* key) {
    ...
    lua_pushnumber(L, id);
    lua_pushstring(L, key);
    if (lua_pcall(L, 2, 1, 0) == 0) { retval = lua_tostring(L, -1); }
    lua_pop(L, 1);
    //}
    return retval;
}
...
const bool libgame_lua_create_dungeonfloor(lua_State* L, const int width, const int height, const int type) {
    ...
    lua_pushnumber(L, type);
    if (lua_pcall(L, 3, 0, 0) == 0) { retval = true; }
    //}
    return retval;
}
...
const int libgame_lua_get_tiletype(lua_State* L, const int x, const int y) {
    ...
    lua_pushnumber(L, x);
    lua_pushnumber(L, y);
    if (lua_pcall(L, 2, 1, 0) == 0) { retval = lua_tonumber(L, -1); }
    lua_pop(L, 1);
    //}
    return retval;
}
...
const bool libgame_lua_entity_move_random_dir(lua_State* L, const int id) {
    ...
    lua_pushnumber(L, id);
    if (lua_pcall(L, 1, 1, 0) == 0) { retval = lua_toboolean(L, -1); }
    lua_pop(L, 1);
    //}
    return retval;
}
...
const int libgame_lua_get_nth_entity_at(lua_State* L, const int n, const int x, const int y) {
    ...
    lua_pushnumber(L, x);
    lua_pushnumber(L, y);
    if (lua_pcall(L, 3, 1, 0) == 0) { retval = lua_tonumber(L, -1); }
    lua_pop(L, 1);
    //}
    return retval;
}
...
const bool libgame_lua_tile_is_occupied_by_npc(lua_State* L, const int x, const int y) {
    ...
    lua_pushnumber(L, x);
    lua_pushnumber(L, y);
    if (lua_pcall(L, 2, 1, 0) == 0) { retval = lua_toboolean(L, -1); }
    lua_pop(L, 1);
    //}
    return retval;
}
...
const bool libgame_lua_create_action(lua_State* L, const int entity_id, const int type, const int x, const int y) {
    ...
    lua_pushnumber(L, x);
    lua_pushnumber(L, y);
    if (lua_pcall(L, 4, 1, 0) == 0) { retval = lua_toboolean(L, -1); }
    lua_pop(L, 1);
    //}
    return retval;
}
