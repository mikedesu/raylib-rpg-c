...
hashtable_entityid_entity_t* hashtable_entityid_entity_create(int size) {
    ...
    ht->size = size;
    ht->table = (hashtable_entityid_entity_node_t**)malloc(sizeof(hashtable_entityid_entity_node_t*) * size);
...
void hashtable_entityid_entity_insert(hashtable_entityid_entity_t* ht, const entityid key, entity_t* value) {
    ...
    const int index = hashtable_entityid_entity_hash(ht, key);
...
void hashtable_entityid_entity_delete(hashtable_entityid_entity_t* ht, const entityid key) {
    const int index = hashtable_entityid_entity_hash(ht, key);
    hashtable_entityid_entity_node_t* node = ht->table[index];
    if (node != NULL) {
        if (node->key == key) {
