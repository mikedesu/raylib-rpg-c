...
static inline void draw_hud(gamestate& g) {
    ...
    snprintf(bf[3], sizeof(bf[3]), "Location: (%d, %d, %d) Turn %d  XP %d", loc.x, loc.y, g.d.current_floor, turn, myxp);
    // Calculate max width of all lines
    const int b0_w = MeasureText(bf[0], font_size), b1_w = MeasureText(bf[1], font_size);
    const int b2_w = MeasureText(bf[2], font_size), b3_w = MeasureText(bf[3], font_size);
    ...
    const int t_x = box_x + g.pad / 2.0f;
    int t_y = box_y + g.pad / 2.0f;
    for (int i = 0; i < 4; i++) {
        DrawText(bf[i], t_x, t_y, font_size, fg);
        t_y += font_size + 2;
    }
}
