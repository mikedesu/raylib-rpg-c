...
shared_ptr<dungeon_floor_t> df_create(int floor, dungeon_floor_type_t t, int width, int height) {
    ...
    df_set_area(df, TILE_STONE_WALL_00, TILE_STONE_WALL_00, (Rectangle){x, y + h / 2.0f, w, 1});

    df_set_area(df, TILE_FLOOR_STONE_00, TILE_FLOOR_STONE_00, (Rectangle){x + w / 2.0f, y + 1, 1, 2});
    ...
    df_set_tile(df, TILE_UPSTAIRS, 10, 10);
    df->upstairs_loc = (vec3){10, 10, floor};

    df_set_tile(df, TILE_DOWNSTAIRS, 9, 14);
    df->downstairs_loc = (vec3){9, 14, floor};
