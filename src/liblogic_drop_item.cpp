...
bool drop_item_from_hero_inventory(shared_ptr<gamestate> g) {
    if (!g->ct.has<inventory>(g->hero_id)) return false;
    ...
    if (item_id == g->ct.get<equipped_weapon>(g->hero_id).value_or(ENTITYID_INVALID)) g->ct.set<equipped_weapon>(g->hero_id, ENTITYID_INVALID);
