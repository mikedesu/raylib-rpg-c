...
static inline void draw_shield_sprite_back(gamestate& g, entityid id, spritegroup_t* sg) {
    ...
    if (shield_back_s)
        DrawTexturePro(*shield_back_s->get_texture(), shield_back_s->src, sg->dest, (Vector2){0, 0}, 0, WHITE);
}
...
static inline void draw_shield_sprite_front(gamestate& g, entityid id, spritegroup_t* sg) {
    ...
    if (shield_front_s)
        DrawTexturePro(*shield_front_s->get_texture(), shield_front_s->src, sg->dest, (Vector2){0, 0}, 0, WHITE);
}
