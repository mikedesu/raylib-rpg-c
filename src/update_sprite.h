...
static inline void libdraw_update_sprite_position(gamestate& g, entityid id, spritegroup* sg) {
    ...
    if (!maybe_sprite_move.has_value()) return;
    ...
    if (sprite_move.x != 0 || sprite_move.y != 0) {
        ...
        if (type == ENTITY_PLAYER || type == ENTITY_NPC) {
            ...
            if (r == RACE_BAT) sg->current = SG_ANIM_BAT_IDLE;
            else if (r == RACE_GREEN_SLIME) sg->current = SG_ANIM_SLIME_IDLE;
            else sg->current = SG_ANIM_NPC_WALK;
        }
        g.frame_dirty = true;
    }
}
...
static inline void libdraw_update_sprite_ptr(gamestate& g, entityid id, spritegroup* sg) {
    ...
    if (g.ct.get<block_success>(id).value_or(false)) libdraw_set_sg_block_success(g, id, sg);
    if (g.ct.get<attacking>(id).value_or(false)) libdraw_set_sg_is_attacking(g, id, sg);
    ...
    if (sg->update_dest()) g.frame_dirty = true;
...
static inline void libdraw_update_sprite_pre(gamestate& g, entityid id) {
    massert(id != ENTITYID_INVALID, "entityid is invalid");
    if (spritegroups.find(id) != spritegroups.end()) libdraw_update_sprite_ptr(g, id, spritegroups[id]);
}
