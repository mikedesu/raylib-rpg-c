...
static inline bool camera_lock_on(gamestate& g) {
    ...
    if (spritegroups.find(g.hero_id) == spritegroups.end()) return false;
    ...
    if (old_target.x != g.cam2d.target.x || old_target.y != g.cam2d.target.y) g.cam_changed = g.frame_dirty = true;
    else g.cam_changed = false;
    return true;
}
